...
## Required parameters
flags.DEFINE_string(
    "bert_config_file", "./bert_config_eth_64.json",
...
def model_fn(features, mode, bert_config, vocab, init_checkpoint, learning_rate,
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        # delete the masked_lm_log_probs, since there are two sequences.
...
def get_masked_lm_output_negative_sampling(bert_config,
    ...
    V = tf.constant(np.arange(word_num), dtype=tf.int64)
    V = tf.expand_dims(V, axis=1)

    if FLAGS.neg_strategy == "uniform":
        neg_ids, _, _ = tf.nn.uniform_candidate_sampler(true_classes=V,
                                                        num_true=1,
        ...
        neg_ids, _, _ = tf.nn.fixed_unigram_candidate_sampler(true_classes=V,
                                                              num_true=1,
                                                              num_sampled=FLAGS.neg_sample_num,
                                                              unique=True,
                                                              range_max=word_num,
...
def main(_):
    ...
    if FLAGS.do_train:
        bert_model, train_op, total_loss = model_fn(features, mode, bert_config, vocab, FLAGS.init_checkpoint, FLAGS.learning_rate,
        ...
        with tf.Session(config=config) as sess:
            ...
            while True:
                try:
                    ...
                    if iter % FLAGS.save_checkpoints_steps == 0 and iter >= 20000:
